...
- **LLM:** `kimi-k2.6` via Moonshot AI platform (OpenAI-compatible SDK)
...
4. Each worker: system prompt + PDF context + worker prompt → streams Markdown from `kimi-k2.6`
...
Prompt caching is automatic on Moonshot's platform — no extra headers needed. Cache primes on the first parallel worker; subsequent workers benefit on the stable prefix (system prompt + PDF context).
